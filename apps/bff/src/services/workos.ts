...
export class WorkOsClient {
   ...
   *   "declined"  WorkOS refused it for a reason specific to this address, such
   *               as an unknown user or one blocked by their own abuse
   *               controls. The CALLER must render this identically to "sent",
   *               and services/magic-auth.ts is where that is enforced.
   *
  ...
  async sendMagicAuthCode(email: string): Promise<"sent" | "declined"> {
    const { status } = await this.#request("/user_management/magic_auth", {
      method: "POST",
      auth: "api-key",
      body: { email },
    });
