...
defined as *"a material breach"* permitting immediate termination.
...
*Note: Last.fm went independent from Paramount Skydance in May 2026. Accounts and API preserved;
no shutdown announced.*

### L2. Apple's iTunes preview terms arguably exclude this app's core use

[Apple's terms](https://performance-partners.apple.com/search-api) permit previews *"only to
promote store content and not for entertainment purposes"*, require content not be used for
*"independent entertainment value apart from its promotional purpose"*, and require previews be
*"streamed only, and not downloaded, saved, cached, or synchronized"*.
...
**Rate limit:** *"approximately 20 calls per minute"* — one call per 3 seconds, **per IP**. This
...
[Terms section IV](https://developers.deezer.com/termsofuse), verbatim: *"The Developer agrees
that the use of the Services is strictly limited for a non-commercial purpose and in a
non-commercial environment."* Developers may not receive revenue related to the service.
...
- `/1/explore/lb-radio` (prompt-based) returns 500: *"currently disabled due to high load"*.
...
Confirmed: *"that rate is (on average) 1 request per second"* — **per IP, not per user**. That
...
`/v1/track/{uuid}/audio-features` needs *that* UUID. Passing a Spotify ID returns 404.
...
Data collection stopped 2022; last update 2022-07-06. MetaBrainz's own assessment: *"the data
simply isn't of high enough quality to be useful for much at all"* — unreliable key and BPM,
...
*Caveat: the Feb 2026 removals were read from the changelog, not confirmed with a live
authenticated token. Verify before designing around them.*
